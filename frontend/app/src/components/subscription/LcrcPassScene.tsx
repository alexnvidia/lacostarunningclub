...
export function LcrcPassScene({ progressRatio, isCompleted, onCompleted }: Props) {
    ...
    useEffect(() => {
        ...
        if (isCompleted) {
            // Activate flag
            if (flagPoleRef.current) gsap.to(flagPoleRef.current,  { fill: cssVar('--t-accent'), duration: 0.5 })
            if (flagCheckRef.current) gsap.to(flagCheckRef.current, { fill: cssVar('--t-bg'), duration: 0.5, delay: 0.1 })
            if (flagBgRef.current)   gsap.to(flagBgRef.current,    { fill: cssVar('--t-fg'), duration: 0.5, delay: 0.1 })
            ...
            >
                <defs>
                    ...
                    <linearGradient id="lcrc-sky" x1="0" y1="0" x2="0" y2="1">
                        ...
                        <stop offset="100%" stopColor="var(--t-bg2)" />
                    </linearGradient>
                    {/* Water shimmer */}
                    <linearGradient id="lcrc-water" x1="0" y1="0" x2="1" y2="0">
                        <stop offset="0%"   stopColor="var(--t-accent)" stopOpacity="0.15" />
                        <stop offset="50%"  stopColor="var(--t-accent2)" stopOpacity="0.20" />
                        <stop offset="100%" stopColor="var(--t-accent)" stopOpacity="0.15" />
                    </linearGradient>
                </defs>
                ...
                {/* Coastal landscape */}
                {/* Moon */}
                <circle cx="700" cy="70" r="30" fill="var(--t-accent2)" opacity="0.12" />
                <circle cx="700" cy="70" r="22" fill="var(--t-accent2)" opacity="0.18" />

                {/* Stars */}
                {[[120,40],[250,25],[480,55],[580,30],[350,15]].map(([x,y],i)=>(
                    <circle key={i} cx={x} cy={y} r="1.5" fill="var(--t-fg)" opacity="0.5" />
                ))}
                ...
                <path d="M0,380 Q40,374 80,380 T160,380 T240,380 T320,380 T400,380 T480,380 T560,380 T640,380 T720,380 T800,380"
                      fill="none" stroke="var(--t-accent)" strokeWidth="1.5" opacity="0.25" />

                {/* Birds/seagulls */}
                <g ref={bgBirdsRef}>
                    <path d="M100,110 Q113,100 126,110 Q139,100 152,110" fill="none" stroke="var(--t-accent)" strokeWidth="2.5" strokeLinecap="round" opacity="0.6" />
                    <path d="M650,60  Q659,52  668,60  Q677,52  686,60"  fill="none" stroke="var(--t-accent2)" strokeWidth="2" strokeLinecap="round" opacity="0.5" />
                    <path d="M380,35  Q386,30  392,35  Q398,30  404,35"  fill="none" stroke="var(--t-accent)" strokeWidth="1.5" strokeLinecap="round" opacity="0.4" />
                </g>
                ...
                <g ref={milestoneRef} transform="translate(750, 110)">
                    <rect ref={flagPoleRef} x="-4" y="0" width="8" height="55" fill="var(--t-border)" rx="2" />
                    {/* Flag */}
                    <rect ref={flagBgRef} x="-4" y="0" width="46" height="28" fill="var(--t-border)" stroke="var(--t-fg-dimmed)" strokeWidth="1.5" rx="1" />
                    <g ref={flagCheckRef} fill="var(--t-fg-dimmed)">
                ...
                <g ref={runnerRef} transform="translate(50,300)">
                    ...
                    <g ref={runnerRigRef}>
                        ...
                        <g ref={armLRef} transform="translate(-10,-10)">
                            <line x1="0" y1="0" x2="-10" y2="20" stroke="var(--t-accent2)" strokeWidth="5" strokeLinecap="round" />
                            <circle cx="-10" cy="20" r="5" fill="var(--t-border)" stroke="var(--t-accent2)" strokeWidth="2" />
                        </g>

                        {/* Back leg */}
                        <g ref={legLRef} transform="translate(-5,20)">
                            <line x1="0" y1="0" x2="0" y2="24" stroke="var(--t-accent)" strokeWidth="6" strokeLinecap="round" />
                            <path d="M-5,24 L5,24 L8,31 L-5,31 Z" fill="var(--t-border)" stroke="var(--t-accent)" strokeWidth="2" />
                        </g>

                        {/* Torso (lightning bolt shape) */}
                        <path d="M-14,-38 L14,-38 L18,-10 L32,-10 L-4,38 L1,10 L-18,10 Z"
                              fill="var(--t-bg2)" stroke="var(--t-accent)" strokeWidth="3" strokeLinejoin="round" />
                        ...
                        <g ref={faceRunRef} transform="translate(0,-14)">
                            <ellipse cx="-6" cy="-5" rx="2" ry="3" fill="var(--t-accent)" />
                            <ellipse cx="6"  cy="-5" rx="2" ry="3" fill="var(--t-accent)" />
                            <path d="M-6,5 Q0,9 6,5" fill="none" stroke="var(--t-accent)" strokeWidth="2" />
                            {/* Sweat drop */}
                        ...
                        <g ref={faceRestRef} transform="translate(0,-14)" style={{ display: 'none' }}>
                            <path d="M-8,-5 Q-6,-8 -4,-5" fill="none" stroke="var(--t-accent)" strokeWidth="2" />
                            <path d="M4,-5 Q6,-8 8,-5"   fill="none" stroke="var(--t-accent)" strokeWidth="2" />
                            <path d="M-6,5 Q0,10 6,5"    fill="none" stroke="var(--t-accent)" strokeWidth="2" />
                        </g>
                    </g>
                </g>
            </svg>
...
function spawnDust(container: SVGGElement | null) {
    ...
    dust.setAttribute('fill', cssVar('--t-fg-dimmed'))
